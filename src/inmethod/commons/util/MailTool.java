...
public class MailTool {
 ...
 /**
  *
  * @param session mail can use  MailTool.buildSession to build mail session
  * @param to receiver's mail account
  * @param cc cc mail account
  * @param bcc bcc  mail account
  * @param from mailler
  * @param subject  subject
  * @param bodytext refer to HtmlMultiPart.class or TextMultiPart.class
  * @param username user name(not yet implement)
  * @param password password (not yet implemnet)
 ...
 public static boolean mailSend(Session session,Vector to,Vector cc,Vector bcc, String from, String subject,
    ...
    if( session == null || to == null || from == null || subject == null || bodytext== null) return false;
  ...
  public static void main(String[] a){
    // to
    Vector to = new Vector();
    to.add("williamc@nexotechnologies.com");
    // cc
    Vector cc = null;
    // bcc
    Vector bcc = null;
    ...
    String subject = "你好";
    // from
    String from = " williamc@nexotechnologies.com";
    // session
    Session session = MailTool.buildSession("192.168.1.186");
    // html with images
    HtmlMultiPart aHMP= new HtmlMultiPart();
